    ...
    <aside class="w-56 bg-white border-r border-gray-200 flex flex-col shrink-0">
      ...
      <div class="px-3 py-3 border-b border-gray-100">
        ...
        <div v-else class="flex items-center gap-2 px-1">
          <div class="w-5 h-5 rounded shrink-0" style="background: linear-gradient(135deg, #C96A3F, #0F2235);" />
      ...
      <nav class="flex-1 px-2 py-3 space-y-0.5">
        <NuxtLink
          ...
          active-class="font-medium hover:bg-[#FEF0E8]"
          style=""
          :style="$route.path === `/dashboard/${currentWorkspace?.slug}` ? 'background:#FEF0E8;color:#C96A3F;' : ''"
        >
        ...
        <NuxtLink
          ...
          :style="$route.path.includes('/integrations') ? 'background:#FEF0E8;color:#C96A3F;' : ''"
        >
          <svg class="w-4 h-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        ...
        <NuxtLink
          ...
          class="flex items-center gap-2.5 px-3 py-2 text-sm rounded-lg text-gray-600 hover:bg-gray-50 hover:text-gray-900 transition-colors"
          :style="$route.path.includes('/billing') ? 'background:#FEF0E8;color:#C96A3F;' : ''"
        >
          <svg class="w-4 h-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        ...
        <NuxtLink
          ...
          class="flex items-center gap-2.5 px-3 py-2 text-sm rounded-lg text-gray-600 hover:bg-gray-50 hover:text-gray-900 transition-colors"
          :style="$route.path.includes('/settings') ? 'background:#FEF0E8;color:#C96A3F;' : ''"
        >
          <svg class="w-4 h-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
